...
def kafka_consumer_thread(kafka_brokers_list, kafka_topic, kafka_group_id):
...
async def startup_event():
    ...
    global KAFKA_BROKERS, KAFKA_TOPIC, KAFKA_CONSUMER_GROUP
    ...
    KAFKA_TOPIC = get_config_from_kv(KAFKA_MESSAGES_TOPIC_KV_KEY, "lab4_messages_topic")
    KAFKA_CONSUMER_GROUP = get_config_from_kv(KAFKA_MESSAGES_GROUP_KV_KEY, "messages_consumers_group")

    thread = threading.Thread(target=kafka_consumer_thread, args=(KAFKA_BROKERS, KAFKA_TOPIC, KAFKA_CONSUMER_GROUP), daemon=True)
    thread.start()
...
@app.get("/health") # Новый эндпоинт для Consul health check
...
if __name__ == "__main__":
    ...
    my_service_id = register_service("messages-service", SERVICE_PORT, health_check_path="/health", is_http_service=True)
