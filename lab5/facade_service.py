...
def initialize_kafka_producer():
    global kafka_producer, KAFKA_BROKERS, KAFKA_TOPIC

    kafka_brokers_str = get_config_from_kv(KAFKA_BROKERS_KV_KEY, "localhost:9092")
    KAFKA_BROKERS = [b.strip() for b in kafka_brokers_str.split(',')] # Обновляем глобальную переменную
    KAFKA_TOPIC = get_config_from_kv(KAFKA_MESSAGES_TOPIC_KV_KEY, "lab4_messages_topic") # Обновляем глобальную переменную
...
async def startup_event():
    ...
    my_service_id = register_service("facade-service", SERVICE_PORT, health_check_path="/health", is_http_service=True)
...
def execute_grpc_call_with_failover(service_name: str, call_func):
    ...
    current_addresses = discover_services(service_name)
    ...
    discover_services("logging-service")
    if last_exception:
...
async def get_all_messages_combined():
    ...
    msg_service_discovered_addresses = discover_services("messages-service")

    if msg_service_discovered_addresses:
        selected_msg_service_address = random.choice(msg_service_discovered_addresses)
