...
def serve(port: int):
    ...
    my_service_id = register_service("logging-service", SERVICE_PORT, is_http_service=False) # TCP check
    hc_members_str = get_config_from_kv(HAZELCAST_MEMBERS_KV_KEY, "127.0.0.1:5701") # Дефолтное значение
    hc_cluster_name = get_config_from_kv(HAZELCAST_CLUSTER_NAME_KV_KEY, "dev")
    hc_members_list = [member.strip() for member in hc_members_str.split(',')]


    try:
        hz_client = hazelcast.HazelcastClient(
            cluster_members=hc_members_list,
